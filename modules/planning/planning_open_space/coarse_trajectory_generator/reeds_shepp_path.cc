...
std::pair<double, double> ReedShepp::calc_tau_omega(
        const double u,
        const double v,
        const double xi,
        const double eta,
...
bool ReedShepp::ShortestRSP(
  ...
  // auto& path = all_possible_paths[optimal_path_index];
  // for (size_t i = 0; i < path.x.size(); i++) {
  //   ssm << path.x[i] << "," << path.y[i] << "," << path.phi[i] << "\n";
  // }
  // ssm << "--rspath\n";
  // AERROR << ssm.str();
...
bool ReedShepp::CS(const double x, const double y, const double phi,
                   std::vector<ReedSheppPath>* all_possible_paths) {
  RSPParam LS1_param;
  LS(x, y, phi, &LS1_param);  // L+S+ L+S-
  double LS1_lengths[2] = {LS1_param.t, LS1_param.u};
  char LS1_types[] = "LS";
  if (LS1_param.flag &&
      !SetRSP(2, LS1_lengths, LS1_types, all_possible_paths, LS1_param.v)) {
    ADEBUG << "Fail at SetRSP with LS_param";
    return false;
  }
  RSPParam LS2_param;
  LS(-x, y, -phi, &LS2_param);  // L-S+ L-S-
  double LS2_lengths[2] = {-LS2_param.t, -LS2_param.u};
  char LS2_types[] = "LS";
  if (LS2_param.flag &&
      !SetRSP(2, LS2_lengths, LS2_types, all_possible_paths, LS2_param.v)) {
    ADEBUG << "Fail at SetRSP with LS_param";
    return false;
  }
  RSPParam RS1_param;
  LS(x, -y, -phi, &RS1_param);  // R+S+ R+S-
  double RS1_lengths[2] = {RS1_param.t, RS1_param.u};
  char RS1_types[] = "RS";
  if (RS1_param.flag &&
      !SetRSP(2, RS1_lengths, RS1_types, all_possible_paths, RS1_param.v)) {
    ADEBUG << "Fail at SetRSP with SLS_param";
    return false;
  }

  RSPParam RS2_param;
  LS(-x, -y, phi, &RS2_param);  // R-S+ R-S-
  double RS2_lengths[2] = {-RS2_param.t, -RS2_param.u};
  char RS2_types[] = "RS";
  if (RS2_param.flag &&
      !SetRSP(2, RS2_lengths, RS2_types, all_possible_paths, RS2_param.v)) {
    ADEBUG << "Fail at SetRSP with SLS_param";
    return false;
  }
  return true;
...
void ReedShepp::LS(const double x, const double y, const double phi,
                   RSPParam* param) {
  double phi_mod = common::math::NormalizeAngle(phi);
  double epsilon = 1e-6;
  if (std::fabs(phi_mod - M_PI_2) < epsilon || std::fabs(phi_mod) < epsilon ||
      std::fabs(phi_mod + M_PI_2) < epsilon ||
      std::fabs(phi_mod + M_PI) < epsilon) {
    return;
  }
  double r = (y - x * std::tan(phi_mod)) / (1 - 1 / std::cos(phi_mod));
  if (r < 1.0) {
    return;
  }
  // u>0 :lpsp ,u<0 :lpsn
  double u = x / std::cos(phi_mod) - r * std::tan(phi_mod);
  double t = phi_mod;
  if (phi_mod < 0) {
    t = phi_mod + 2 * M_PI;
  }
...
bool ReedShepp::GenerateLocalConfigurations(
  ...
  for (size_t i = 0; i < shortest_path->segs_types.size(); ++i) {
    ...
    while (std::abs(pd) <= std::abs(l)) {
      index++;
      Interpolation(index, pd, m, ox, oy, ophi, radius, &px, &py, &pphi,
                    &pgear);
      pd += d;
    }
    ll = l - pd - d;
...
void ReedShepp::Interpolation(const int index, const double pd, const char m,
                              ...
                              const double ophi, const double radius,
  ...
  if (m == 'S') {
    ...
    ldx = std::sin(pd) / max_kappa_ * radius;
    if (m == 'L') {
      ldy = (1.0 - std::cos(pd)) / max_kappa_ * radius;
    } else if (m == 'R') {
      ldy = (1.0 - std::cos(pd)) / -max_kappa_ * radius;
    }
